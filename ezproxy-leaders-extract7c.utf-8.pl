...
# дата для извлечения из файла лога инфы за месяц
my $strdate_argv = $ARGV[0]  # читаем входную дату, из первого аргумента командной строки
   or die "Usage: script.pl YYYY-mm";
# входная дата, в формате YYYY-mm
...
# дата для парсинга лога, в формате Mmm/YYYY
...
#   $pattern .= q|\s+([a-zA-Z0-9]{15})|;                                                 # ID сессии
   ...
   $pattern .= q|\s+\[\d{2}\/|.$strdate_parsing.q|\:(\d{2}\:\d{2}\:\d{2})\s{1}\+\d{4}\]|;      # дата-время (возвращается только время, без даты)
   $pattern .= q|\s+["][^"]+["]|;                                                       # URL
...
# выводим
  ...
  if (@ips_desc == 1) {
    ...
    # а по трафику
    if ($hosts1{$key}{SIZE}) { # не нуль -- высчитываем процентный вклад в каждый IP-адрес
